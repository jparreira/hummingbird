...
Site tracking and analytics storage using the Realtime Framework Messaging Service
...
This fork of Hummingbird uses the Realtime Framework Messaging Service to broadcast the metrics data. It has been modified to optimize the number of messages sent (only sends data to the dashboard clients when there's new data available).

You can use the built-in default shared license, `2Ze1dz`, but to keep your metrics private you should get your own Realtime.co Free Developers License at [https://app.realtime.co/developers/getlicense](https://app.realtime.co/developers/getlicense)
...
--------------
    ...
    git clone git://github.com/jparreira/hummingbird.git
    cd hummingbird
...
falls back long polling if necessary.
